...
require([
    ...
    $.each (apiByGroupAndName, function(index, groupEntries) {
        ...
        //Custom order Get, Save, Update, Delete
        for (var i = 0; i < titles.length; i++){
            if (titles[i].startsWith("G")) titles[i] = "0" + titles[i];
            if (titles[i].startsWith("S")) titles[i] = "1" + titles[i];
            if (titles[i].startsWith("U")) titles[i] = "2" + titles[i];
            if (titles[i].startsWith("D")) titles[i] = "3" + titles[i];
        }
