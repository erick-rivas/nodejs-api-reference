...
import { MOCK_TEMPLATE, MOCK_ITEM_TEMPLATE } from "@support/gens/models/react/Templates";
  ...
  async generate()
  {
    ...
    for (let c of this.classes)
      for (let co in this.consts[c]) {
        imports += `import { ${co} } from "models/util/Const";\n`;
        hasConst = true;
      }
    if (hasConst)
      imports += `import { getEnum } from "model/util/Const";\n`;
    ...
    super.generateFile("/models/util", "Mocks.ts", res);
